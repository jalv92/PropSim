...
description: >
  ...
  system-ui in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
colors:
  # PropSim remix — the product's own UI tokens (dashboard.html :root), verbatim.
  # `fire-orange` keeps the preset's KEY NAME (prose and components reference it)
  # but holds PropSim's signal green: the sole accent on the dark register.
  ink-black: "#0b0b0d"
  ink-black-alt: "#121316"
  fire-orange: "#34d399"
  cream: "#e7e9ea"
  cream-muted: "#8b9096"
  cream-hint: "#5f646b"
  border-dark: "#232529"
  # — semantic pair: NOT decoration. Meaning is encoded here and nowhere else. —
  signal-red: "#f2635f"
  signal-amber: "#f0b429"
  signal-blue: "#5aa9e6"
  ink-on-orange-muted: "rgba(11,11,13,0.75)"
  ink-on-orange-hint: "rgba(11,11,13,0.55)"
  ink-on-orange-faint: "rgba(11,11,13,0.40)"
  ink-on-orange-border: "rgba(11,11,13,0.20)"

typography:
  # — reading ramp —
  body:    { fontFamily: "system-ui", cqw: 1.2, weight: 400, lineHeight: 1.6 }
  lead:    { fontFamily: "system-ui", cqw: 1.6, weight: 400, lineHeight: 1.5 }
  caption: { fontFamily: "system-ui", cqw: 0.9, weight: 400, lineHeight: 1.5 }
  label:   { fontFamily: "IBM Plex Mono", cqw: 0.72, weight: 500, tracking: "0.14em", upper: true }
  # — display / hero ramp (system-ui, lowercase, negative tracking) —
  h3:      { fontFamily: "system-ui", cqw: 2.8, weight: 600, lineHeight: 1.2, lower: true }
  quote-text:{ fontFamily: "system-ui", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.02em", lower: true }
  h2:      { fontFamily: "system-ui", cqw: 4.5, weight: 700, lineHeight: 1.1, tracking: "-0.02em", lower: true }
  stat-value:{ fontFamily: "ui-monospace, SFMono-Regular, Menlo, Consolas, monospace", cqw: 5.5, weight: 700, lineHeight: 1.0, tracking: "-0.03em" }
  h1:      { fontFamily: "system-ui", cqw: 7.5, weight: 800, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  fadelist-item:{ fontFamily: "system-ui", cqw: 7.5, weight: 900, lineHeight: 1.0, tracking: "-0.03em", lower: true }
  quote-mark:{ fontFamily: "system-ui", cqw: 10.0, weight: 900, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "system-ui", cqw: 10.5, weight: 900, lineHeight: 0.9, tracking: "-0.04em", lower: true }
  display: { fontFamily: "system-ui", cqw: 13.0, weight: 900, lineHeight: 0.88, tracking: "-0.04em", lower: true }
...
- **Fonts** — already set to **ui-monospace** (display) / **system-ui** (body); ignore any preset font name lingering in prose.
- **Colors** — use the frontmatter hex; preset color NAMES in prose (e.g. "cobalt", "cream") mean the remapped brand values.

### PropSim overrides — these BEAT the preset prose below

1. **One register only: dark.** There is no orange/flooded register in this video. PropSim's
   ground is `{colors.ink-black}` in every frame, exactly as the product's own UI is. Wherever
   the prose below says "ORANGE register" or "ground fire-orange", read: dark ground, and the
   declaration is carried by **scale** instead of a ground flip. A full-bleed flood of the accent
   would look nothing like the tool.
2. **`fire-orange` is PropSim's signal green `#34d399`** — the sole accent, exactly as the preset
   rations its single color. Every prose mention of orange means this green.
3. **Two extra colors exist, and they are meaning, not palette.** `{colors.signal-red}` = an
   account that breached — a rule broken, a path dead. `{colors.signal-amber}` = a caveat the
   tool refuses to hide (unverified rule, thin sample, trials burned). Neither may be used
   decoratively, for emphasis, or because a frame "needs another color". A frame with red in it
   is a frame about something failing. That restraint IS the brand: the product's whole pitch is
   that it doesn't flatter you.
4. **Every number is monospace.** Counts, probabilities, dollar figures, dates — `stat-value` and
   any inline figure. The product renders all of them in mono and so does this video.
5. **Curves are drawn, never illustrated.** Equity paths, floors, histograms and the donut are
   real SVG geometry. No raster imagery ships in this composition.


## Overview

Broadside at frame scale is a **protest-poster system where type is so large it stops reading as
text and becomes graphic primitive.** system-ui `display` at 13cqw puts a single lowercase word
...
**system-ui** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase system-ui 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (system-ui body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (system-ui `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **system-ui display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
system-ui `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
system-ui lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word system-ui `display`/`h1`
...
row of three top-border-only stat-cards — big system-ui-900 numeral in `{colors.fire-orange}`, system-ui
...
**Ground** ink-black. **Composes** fadelist (3 stacked system-ui-900 words at 1.0/0.5/0.22), fadelist-title.
...
system-ui `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every system-ui display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase system-ui display; never add a second accent color.
...
- No serif companion; chrome is never system-ui.
...
- **Type** — system-ui lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **system-ui + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
